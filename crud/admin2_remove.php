...
if(!isset($_SESSION['user']))
{
    header('Location:login.php');
        ...
        $conn=mysqli_connect('localhost','root','','db'); 
         ...
         if(isset($_GET['admin2_id']) && $_GET['image'])
            {
                $getIamgeName = $_GET['image'];
                $getId = $_GET['admin2_id'];
                $selectSql = "SELECT * FROM `admin2` WHERE `admin2_id`='$getId' ";
                ...
                // exit();
                            ...
                            {
                                 $deleteSql = "UPDATE `admin2` SET `file`='$getImageName' WHERE `admin2_id`='$getId'";
